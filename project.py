...
# Snake config
CELL = 32                 # was 20; now larger snake and food
SNAKE_INIT_LEN = 5
SNAKE_STEP_FRAMES = 12    # was 6; now slower movement
SNAKE_MIN_LEN = 3
...
def draw_food():
    ...
    # special sphere (bonus)
    if food_special:
        glPushMatrix()
        glTranslatef(food_special[0], food_special[1], food_special[2])
        glColor3f(1.0, 1.0, 0.0)  # yellow
        gluSphere(gluNewQuadric(), CELL * 0.8, 20, 20)  # larger sphere
        glPopMatrix()
...
def reset_game():
    ...
    # spawn foods with new rarity logic
    food_special = spawn_food_away_from_snake()  # yellow, most common
    food_normal = None
    food_shrink = None
    # 70% chance for yellow, 20% orange, 10% red
    roll = random.random()
    if roll < 0.4:
        food_normal = spawn_food_away_from_snake()  # orange, rare
    elif roll < 0.5:
        food_shrink = spawn_food_away_from_snake()  # red, very rare
...
def move_snake_one_step():
    ...
    if ate_special:
        ...
        # 70% chance to spawn another yellow, 20% orange, 10% red
        roll = random.random()
        food_normal = None
        food_shrink = None
        if roll < 0.2:
        ...
        score += 1
        food_normal = None
        # 70% chance to spawn yellow, 20% orange, 10% red
        roll = random.random()
        if roll < 0.7:
            food_special = spawn_food_away_from_snake()
        elif roll < 0.9:
        ...
        snake.pop() if len(snake) > SNAKE_MIN_LEN else None
        snake.pop() if len(snake) > SNAKE_MIN_LEN else None
        food_shrink = None
        # 70% chance to spawn yellow, 20% orange, 10% red
        roll = random.random()
