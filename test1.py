...
grid_rot_horizontal = 90  # left/right (initially 90 degrees left)

# Grid color state
grid_colors_randomized = False
grid_block_colors = None
grid_block_colors_prev = None
grid_rot_vertical = 0    # up/down
...
def showScreen():
    ...
    status_lines = []
    if cheat_mode:
        status_lines.append("Cheat Mode: ON (g)")
    else:
        status_lines.append("Cheat Mode: OFF (g)")
    if snake_color_randomized:
        status_lines.append("Snake Color: RANDOM (c/v)")
    else:
        status_lines.append("Snake Color: DEFAULT (c/v)")
    if grid_colors_randomized:
        status_lines.append("Grid Color: RANDOM (h)")
    else:
        status_lines.append("Grid Color: DEFAULT (h)")
    status_lines.append(f"Grid Rot: Hor={grid_rot_horizontal}°  Vert={grid_rot_vertical}° (w/a/s/d)")
    status_lines.append(f"Snake Speed: {SNAKE_SPEED_FACTOR:.1f} (+ = faster, - = slower)")
    # Draw each line, right-aligned
    x_right = 700
    y_top = 770
    for i, line in enumerate(status_lines):
        draw_text(x_right, y_top - i*30, line)
